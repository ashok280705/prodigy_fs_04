  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-0 bg-black">
        <div className="absolute bottom-0 left-[-20%] right-0 top-[-10%] h-[100vh] w-[500px] rounded-full bg-[radial-gradient(circle_farthest-side,rgba(255,0,182,.15),rgba(255,255,255,0))]" />
      ...
      <main className="relative z-10 min-h-screen text-white mt-16 container mx-auto">
        <div
          className={`p-4 md:p-8 transform transition-all duration-700 ${
              ...
              <div className="hidden sm:block">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-w-7xl">
                  {rooms.map((room, index) => (
                    <div
                      key={room._id}
                      className={`group bg-gray-900/80 backdrop-blur-sm border border-gray-700/50 rounded-xl overflow-hidden hover:border-pink-500/30 hover:shadow-lg hover:shadow-pink-500/10 transition-all duration-300 transform ${
                        isLoaded
                          ...
                          <div className="w-12 h-12 mx-auto mb-2 bg-white/10 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                            <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path
                                ...
                                strokeLinejoin="round"
                                strokeWidth={2}
                                d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a2 2 0 01-2-2v-2M3 4h6v4H3z"
                              />
                            </svg>
                          </div>
                        </div>
                ...
                <div className="overflow-x-auto pb-4">
                  <div className="flex space-x-4" style={{ minWidth: "max-content" }}>
                    {rooms.map((room, index) => (
                      <div
                        key={room._id}
                        className={`group bg-gray-900/80 backdrop-blur-sm border border-gray-700/50 rounded-xl overflow-hidden hover:border-pink-500/30 hover:shadow-lg hover:shadow-pink-500/10 transition-all duration-300 flex-shrink-0 w-72 transform ${
                          isLoaded
                            ? "translate-y-0 opacity-100 scale-100"
                            : "translate-y-8 opacity-0 scale-95"
                        }`}
                      ...
                      >
                        ...
                        <div className="p-4">
                          ...
                          <p className="text-xs text-gray-400 mb-4 truncate">ID: {room.roomId}</p>
                          <button
                            onClick={() => handleJoin(room.roomId)}
